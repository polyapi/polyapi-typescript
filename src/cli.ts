...
void yargs
  ...
  .command('setup [baseUrl] [apiKey]',
    ...
    async (argv) => {
      ...
      await setup(DEFAULT_POLY_PATH, argv.baseUrl, argv.apiKey, argv.apiVersion);
    },
  )
  .command(
    'generate [options]',
    ...
    async ({ exitWhenNoConfig, contexts, names, functionIds, customPath = DEFAULT_POLY_PATH, noTypes = false }) => {
  ...
  .command('prepare [options]', 'Find and prepare all Poly deployables', (yargs) =>
    yargs
  ...
  async ({ disableDocs, disableAi, lazy }) => {
    if (!checkPolyConfig(DEFAULT_POLY_PATH)) {
      return shell.echo('Poly is not configured. Please run `poly setup` to configure it.');
    }
  ...
  .command('sync [options]', 'Find and sync all Poly deployables', (yargs) =>
    yargs
  ...
  .command('function <command>', 'Manages functions', (yargs) => {
    yargs.command(
      ...
      async ({ name, description, file, context, client, server, logs, generateContexts, executionApiKey }) => {
        const logsEnabled = logs === 'enabled' ? true : logs === 'disabled' ? false : undefined;
        const err =
          !name
    ...
    yargs.command(
      ...
      async ({
        ...
        for (const mappings of rename) {
          ...
          if (!prevName || !newName) {
            shell.echo(chalk.redBright('ERROR:'), `Invalid rename mapping from "${prevName}" to "${newName}".`);
            yargs.showHelp();
            return;
          }
        ...
        await generateModel(path, destination, context, hostUrl, hostUrlAsArgument, !!disableAi, preparedRenames);
      },
  ...
  .command('snippet <command>', 'Manage snippets.', async (yargs) => {
    yargs.command(
      ...
      async ({
        ...
        await addSnippet(DEFAULT_POLY_PATH, name, path, context || '', description || '');
      },
    );
  })
