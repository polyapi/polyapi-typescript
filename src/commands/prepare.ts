...
import { getClientFunctionDescription, getServerFunctionDescription } from '../api';

const getFunctionDescription = (type: DeployableTypes, description: string, args: FunctionArgumentDto[], code: string) =>
...
const fillInMissingFunctionDetails = async (deployable: DeployableRecord, code: string): Promise<DeployableRecord> => {
  const isMissingDescriptions = !deployable.types.description ||
  ...
  if (isMissingDescriptions) {
    try {
      const aiGenerated = await getFunctionDescription(
        deployable.type, deployable.types.description, deployable.types.params.map(p => ({ ...p, key: p.name })), code);
...
const fillInMissingDetails = async (deployable: DeployableRecord, code: string): Promise<DeployableRecord> => {
...
const getAllDeployables = async (disableDocs: boolean, disableAi: boolean, gitRevision: string): Promise<DeployableRecord[]> => {
  ...
  console.log(`Found ${possibleDeployables.length} possible deployable file${possibleDeployables.length === 1 ? '' : 's'}.`);
  ...
  for (const possible of possibleDeployables) {
    try {
      const [deployable, code] = await parseDeployable(possible, baseUrl, gitRevision);
      const fullName = `${deployable.context}.${deployable.name}`;
      if (found.has(fullName)) console.error(`Prepared ${deployable.type.replaceAll('-', ' ')} ${fullName}: DUPLICATE`);
      else {
        found.set(fullName, disableAi || deployable.disableAi || deployable.type === 'webhook' ? deployable : await fillInMissingDetails(deployable, code));
        console.log(`Prepared ${deployable.type.replaceAll('-', ' ')} ${fullName}: ${deployable.dirty && !disableDocs ? 'UPDATED' : 'OK'}`);
      }
    } catch (err) {
...
export const prepareDeployables = async (lazy: boolean, disableDocs: boolean, disableAi: boolean) => {
  ...
  const parsedDeployables = await getAllDeployables(disableDocs, disableAi, gitRevision);
  ...
  if (dirtyDeployables.length) {
    ...
    console.log(`Fixing ${dirtyDeployables.length} deployable file${dirtyDeployables.length === 1 ? '' : 's'}.`);
