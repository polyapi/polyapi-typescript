...
const getDuplicatedIdentifier = (apiFunctions: { name: string, context: string }[]): string | null => {
  for (let i = 0; i < apiFunctions.length; i++) {
    ...
    for (let c = 0; c < apiFunctions.length; c++) {
      ...
      if (firstApiFunction.name && firstApiFunction.context === secondApiFunction.context && firstApiFunction.name === secondApiFunction.name && c !== i) {
        return firstApiFunction.context ? `${firstApiFunction.context}.${firstApiFunction.name}` : firstApiFunction.name;
      }
    }
  }
...
export const validateModel = async (path: string) => {
  ...
  try {
    ...
    if ('functions' in specificationInput) {
      const duplicatedIdentifier = getDuplicatedIdentifier(specificationInput.functions);
    ...
    if ('webhooks' in specificationInput) {
      const duplicatedIdentifier = getDuplicatedIdentifier(specificationInput.webhooks);
    ...
    if (error instanceof DuplicatedIdentifierError) {
      shell.echo(chalk.red('Error:'), error.message, chalk.red(error.identifier));
