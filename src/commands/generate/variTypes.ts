...
const normalizeVariSchema = (schema: JsonSchema): JsonSchema => {
  ...
  if (normalized.definitions) {
    const mapping = new Map(
      Object.entries(normalized.definitions).map(([name, definition]) => [
        `#/definitions/${name}`,
        definition,
      ]),
    );
...
export const getVariableValueTypeDeclarations = (
  ...
  const valueTypeDeclaration = printSchemaAsType(
    normalized,
    'ValueType',
    1,
    null,
