...
export type JsonSchema = {
  ...
  discriminator?: { // technically this is part of OpenAPI--not JSON Schema
    propertyName: string;
...
export type SchemaSpec = Omit<SchemaSpecification, 'definition'> & { definition: JsonSchema; };

type SchemaTree = Record<string, SchemaSpec | Record<string, SchemaSpec>>;

const ws = memoize((depth = 1) => depth < 0 ? '' : new Array(depth).fill('  ').join(''));
const end = memoize((nested?: NestedT) => !nested || nested === 'object' ? ';' : '');
...
const printComment = (comment = '', depth = 0, deprecated = false) => {
  ...
  if (!comment && deprecated) {
    return `${ws(depth)}/**${EOL}${ws(depth)} * @deprecated${EOL}${ws(depth)} */${EOL}`;
  }
...
type PrintSchemaFn = (schema: JsonSchema, key: string, depth: number, nested?: NestedT, optional?: boolean) => string;

const printTypeName = (title: string | undefined, key: string, nested: NestedT, optional = false) => {
  if (!nested) {
...
const printEnumSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  const values = schema.enum || [];
  if (values.length === 1) {
    return printConstSchema({ ...schema, const: values[0] }, key, depth, nested, optional);
  }
  if (schema.nullable && !values.includes(null)) values.unshift(null);
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }${values.map(v => `${EOL}${ws(depth + 1)}| ${typeof v === 'string' ? `'${v}'` : v}`).join('')
    }${end(nested)}`;
};

const printStringSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
...
const printNumberSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }${schema.nullable ? 'null | ' : ''
...
const printBooleanSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }${schema.nullable ? 'null | ' : ''
...
const printNullSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }null${end(nested)}`;
...
const printObjectSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  let result = `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }{`;
  if (schema.properties || schema.patternProperties || schema.additionalProperties) {
    if (schema.properties) {
      Object.entries(schema.properties).forEach(([k, v]) => {
        result = `${result}${EOL}${printSchemaAsType(v, k, depth + 1, 'object', !schema.required?.includes(k))}`;
      });
    }
    if (schema.patternProperties) {
      ...
      result = `${result}${EOL}${printSchemaAsType(subschema, '[k: string]', depth + 1, 'object')}`;
    } else if (schema.additionalProperties) {
      if (typeof schema.additionalProperties === 'object') {
        result = `${result}${EOL}${printSchemaAsType(schema.additionalProperties, '[k: string]', depth + 1, 'object')}`;
      } else {
...
const printTupleSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  ...
  if (!schema.items.length) {
    ...
    if (schema.additionalItems) {
      if (typeof schema.additionalItems === 'object') {
        const child = printSchemaAsType(schema.additionalItems, '', depth + 1, 'array').trim();
...
const printArraySchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
...
const printMultiTypeSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  if (!Array.isArray(schema.type) || !schema.type.length) throw new Error('schema.type should be a non-empty array to use this function');
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }${schema.type.join(' | ')
...
const printIntersectionSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  if (!Array.isArray(schema.allOf) || !schema.allOf.length) throw new Error('schema.allOf should be a non-empty array to use this function');
  if (schema.allOf.length === 1) {
    // no need to print as an intersection type since only one value
    return printSchemaAsType({ ...schema, ...schema.allOf[0], allOf: undefined }, key, depth, nested, optional);
  }
...
const printUnionSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  if (!Array.isArray(schema.anyOf) || !schema.anyOf.length) throw new Error('schema.anyOf should be a non-empty array to use this function');
  if (schema.anyOf.length === 1) {
    // no need to print as a union type since only one value
    return printSchemaAsType({ ...schema, ...schema.anyOf[0], anyOf: undefined }, key, depth, nested, optional);
  }
  const subtypes = schema.anyOf
    .map(s => printSchemaAsType(s, '', depth + 1, 'union'))
    .map((v, i) => `${EOL}${ws(depth + 1)}${i || !nested || nested === 'object' ? '| ' : ''}${wrapParens(v.trim())}`)
    .join('');
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
...
const printPolyRefSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }${schema['x-poly-ref'].publicNamespace ? `${schema['x-poly-ref'].publicNamespace}.` : ''}${schema['x-poly-ref'].path.split('.').map(v => formatName(v)).join('.')
    }${end(nested)}`;
};

const printAnySchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
    }unknown${end(nested)}`;
...
const printUnresolvedSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
...
const printConstSchema: PrintSchemaFn = (schema, key, depth, nested, optional = false) => {
  return `${printComment(schema.description, depth, schema.deprecated)
    }${ws(depth)
    }${printTypeName(schema.title, key, nested, optional)
...
const printSchemaAsType: PrintSchemaFn = (schema: JsonSchema, key, depth = 0, nested, optional = false): string => {
...
const printSchemaTreeAsTypes = (schema: SchemaTree, name: string, depth = 0): string => {
  ...
  for (const [key, child] of Object.entries(schema)) {
    ...
    if ('type' in child && child.type === 'schema') {
      try {
        generated = printSchemaAsType(child.definition as JsonSchema, (child.name || key) as string, depth + 1);
      } catch (err) {
        console.error(err);
  ...
  for (const spec of specs) {
    ...
    if (spec.unresolvedPolySchemaRefs?.length) {
      for (const unresolved of spec.unresolvedPolySchemaRefs) {
        ...
        const fillerSpec: SchemaSpec = {
          ...
          definition: { type: 'unresolved', title: name, description: `Unresolved schema, please add schema \`${unresolved.path}\` to complete it.` },
          visibilityMetadata: {
    ...
    for (const contextName of refs) {
      ...
      const fillerSpec: SchemaSpec = {
        ...
        context,
        contextName,
        definition: { type: 'unresolved', title: name, description: `Missing schema, as context \`${context}\` was not generated.` },
        visibilityMetadata: {
          // @ts-expect-error - it's fine
          visibility: 'ENVIRONMENT',
        },
...
const printSchemaSpecs = (specs: SchemaSpec[]): Record<string, string> => {
  ...
  const fileMap = Object.fromEntries(trees.map(tree => [`${tree.path}.d.ts`, printSchemaRoot(tree)]));
  fileMap['index.d.ts'] = Object.keys(fileMap).map(file => `/// <reference path="./${file}" />`).join(EOL);
  return fileMap;
};

export const generateSchemaTSDeclarationFiles = async (libPath: string, specs: SchemaSpec[]): Promise<void> => {
